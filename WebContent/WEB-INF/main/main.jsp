...
<script type="text/javascript">
	$(function() {
		$("a[title]").click(function() {
			...
			alert(url);
			//判断是否存在
			if($('#tt').tabs("exists", this.title)){ //存在则选中面板, select
			//存在则选中
				alert("存在了");
				$('#tt').tabs("select", this.title);			
			}else{
				$('#tt').tabs('add', { //不存在则add一个面板
					title:this.title,
					//面板有关闭按键
				    closable:true, 
				    //href对远程页面js的支持不好 
					...
					content:"<iframe src='"+url+"' title='"+this.title+"' height='100%' width='100%' frameborder='0px' ></iframe>"
				});
			}
			return false; //超链接的地址是不用去的!所以是false
		});
	});
</script>
...
<body class="easyui-layout">
	<!-- 头部 -->
	<div data-options="region:'north',title:'欢迎使用进销存管理系统',split:true" style="height:130px;">欢迎您 ：${sessionScope.account.accLogin}</div>
			...
			<div title="基础数据录入">
				...
				<ul style="list-style: none;padding: 0px;margin:0px;">
					<li style="padding: 6px;">
					    <a href="${proPath}/base/goURL/supplier/supplierlist.action" title="供应商管理"
								style="text-decoration: none; display: block;font-weight:bold;">供应商管理
						</a>
					</li>
					<li style="padding: 6px;">
						<a href="${proPath}/base/goURL/goods/goodslist.action" title="商品管理"
								style="text-decoration: none;display: block;font-weight:bold;">商品管理
						</a>
					</li>
					
					<li style="padding: 6px;">
						<a href="${proPath}/base/goURL/organization/organizationlist.action" title="机构管理"
								style="text-decoration: none;display: block;font-weight:bold;">机构管理
						</a>
					</li>
				</ul>
			</div>
